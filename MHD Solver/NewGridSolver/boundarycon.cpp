...
void ConvBound()
{
    ...
    // Original variables are defined as follows - J is for radial and K is for length-wise
    ...
    for(J=Rc+1; J<Ra; J++)
    {
        ...
        Rho[J][0]= mdot/(Vz[J][0]*PI*(pow(Ranode[0],2.0)
        -pow(Rcathode[0],2.0)));
        n[J][0] = Rho[J][0]/mAr;
        ne[J][0] = 0.999*n[J][0];
        ...
        +(0.5*BSq[J][0]/Mu); // ?? should the p term be there
...
void DissBound()
{
    ...
    for(J=Rc+1; J<=Ra; J++)
    {
        ...
        DissipZ[3][J][0] = Vappl/(log(Ranode[0]/Rcathode[0])
        *((J-0.5)*deltaR));
        DissipZ[4][J][0] = -((-Vappl/(log(Ranode[0]/Rcathode[0])
        *((J-0.5)*deltaR)) )
