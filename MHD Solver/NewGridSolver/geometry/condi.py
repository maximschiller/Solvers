...
radius = 10
cyllen = 30
tipstart = 25-np.sqrt(20**2 - 10**2) # for cathode
tiprad = 5
Rt = 6 # in mm - just an assumption for now - throat radius is actually additionally offset by radius
...
# concatenating coordinates for plotting
x = np.concatenate([xent, xex, xbell], axis=0)
y = np.concatenate([yent, yex, ybell], axis=0)
z = np.zeros_like(x) # creates an array of zeros with the same length as x and y
...
# radius and cyllen are of the cylinder part of the nozzle
diffx = cyllen-x[0]
diffy = radius-y[0]
# diffx = 0
# diffy = 0
## Writing the coordinates to a tab delimited text file for solidworks
# Open the output file in write mode
with open(r'solidworksgeom/condi_cad.txt', 'w') as f:
    # Write header row
    # f.write('x\ty\n')
    
    # Write data rows
    for i in range(len(x)):
        f.write('{}\t{}\t{}\n'.format(x[i]+diffx, y[i]+diffy, z[i]))


## Generating the rest of the data points for meshing
# nozzle should join the main part of the anode at x = 30 and y = 25
# NOTE only considering first quadrant due to axisymmetry

# generating top boundary
xcyl = np.linspace(0,cyllen,spacing)
ycyl = np.ones_like(xcyl)*radius

xtop = np.concatenate([xcyl, x+diffx])
ytop = np.concatenate([ycyl, y+diffy])
xnegtop = xtop
ynegtop = -ytop


# z = np.zeros_like(xtop) # creates an array of zeros with the same length as x and y
# ## Writing the coordinates to a tab delimited text file for solidworks
# # Open the output file in write mode
# with open(r'solidworksgeom/condi_cad.txt', 'w') as f:
#     # Write header row
...
xbotexit = np.linspace(20,x[-1]+diffx,spacing)
...
plt.savefig(r'C:\Users\maxim\OneDrive - The University of Sydney (Students)\THESIS\Thesis B figs' + '/condi.png')
